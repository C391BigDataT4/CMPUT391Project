...
def main(argv):
    ...
    query_4 = "SELECT GROUP_AND_COUNT(CITY_ID) FROM CDR WHERE MOBILE_ID_TYPE = 1 AND CITY_ID IN (0,1,2) AND (MONTH_DAY, MSC_CODE) >= (1,10) AND (MONTH_DAY, MSC_CODE) <= (31,20) LIMIT 400"
    query_5 = "SELECT GROUP_AND_COUNT(MONTH_DAY) FROM CDR WHERE MOBILE_ID_TYPE = 2 AND (CITY_ID, MONTH_DAY, MSC_CODE, SERVICE_NODE_ID, SEQ_NUM) >= (1,1,30,7,100) AND (CITY_ID, MONTH_DAY, MSC_CODE, SERVICE_NODE_ID, SEQ_NUM) <= (1,31,50,9,200)"
